...
Track::Initialize(void)
{
    ...
    // I can use this to draw new quads I think
    ...
    for (i = 0; i <= n_refined; i++)
    {
        ...
        glPushMatrix();
        glTranslatef(p[0], p[1], p[2]-0.5);

        angle = atan2(p[1], p[0]) * 180 / M_PI;
    ...
    for (i = 0; i <= n_refined; i++)
    {
        ...
        gluCylinder(tracks, radius, radius, p[2]-0.5, numSlices, numStacks);
        glPopMatrix();
        /* // couldnt get the other cylinders to be in the correct location
        glPushMatrix();
        angle = (atan2(p[1], p[0]) * 180 / M_PI) + 90;
        adj = (1.0f) * cos((angle) * M_PI/ 180);// always x-direction
        opp = (1.0f) * sin((angle) * M_PI /180);// always y-direction

        if (angle >= 90) { // this i q4???
            glTranslatef(p[0] - adj, p[1] + opp, 0.0);
        }
        else if (angle >= 180) {
            glTranslatef(p[0] - adj, p[1] + opp, 0.0);
        }
    ...
    // edit - this is where I will implement the new train cart
...
void
Track::Evaluate_Position(float position[3], float tangent[3]) {
    this->track->Evaluate_Point(this->posn_on_track, position);
    this->track->Evaluate_Derivative(this->posn_on_track, tangent);
    Normalize_3(tangent);
}