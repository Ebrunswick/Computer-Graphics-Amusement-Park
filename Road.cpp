...
// Custom Constructor
Road:: Road(GLdouble specified_path_length, GLdouble specified_path_width, float x, float y, float z) {
    display_list = 0;
    initialized = false;
    this->path_length   = specified_path_length;
    this->path_width    = specified_path_width;
    this->x_position    = x;
    this->y_position    = y;
    this->z_position    = z;
}
...
Road::Initialize(void)
{
    ...
    if (!(image_data = (ubyte*)tga_load("redBrick.tga", &image_width,
        &image_height, TGA_TRUECOLOR_24)))
    {
        fprintf(stderr, "Road::Initialize: Couldn't load asphalt.tga, time to load a different object.\n");
        return false;
    }
    ...
    path_length = 20;
    path_width = 10;
    this->x_position = 30.0f;
    this->y_position = 0.0f;
    this->z_position = 0.125f;
    */

    glBegin(GL_QUADS);
    glTexCoord2f(1, 1);
    glVertex3f(path_length, path_width, 0.0);
    glTexCoord2f(-1, 1);
    glVertex3f(-path_length, path_width, 0.0);
    glTexCoord2f(-1, -1);
    glVertex3f(-path_length, -path_width, 0.0);
